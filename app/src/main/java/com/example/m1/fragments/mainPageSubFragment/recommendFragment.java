...
//        animInit();
    ...
    private void adapterInit() {
        ...
        recycleView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                                            ...
                                            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                                                ...
                                                // 在newState为滑到底部时
                                                if (newState == RecyclerView.SCROLL_STATE_IDLE && moreFlag==true) {
                                                    // 如果没有隐藏footView，那么最后一个条目的位置就比我们的getItemCount少1，自己可以算一下
                                                    if (!recommendVideoAdapter.isFadeTips() && lastVisibleItem + 1 == recommendVideoAdapter.getItemCount()) {
                                                    ...
                                                    if (recommendVideoAdapter.isFadeTips() && lastVisibleItem + 2 == recommendVideoAdapter.getItemCount()) {
                                                        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                                                            @Override
                                                            public void run() {
                                                                // 然后调用updateRecyclerview方法更新RecyclerView
    ...
    private void updateRecyclerView() {
        ...
        if (newDatas.size()<=PAGE_COUNT && fromIndex==0){
            toIndex=newDatas.size();
        }
        if (newDatas.size() > 0) {
            if(newDatas.size()==PAGE_COUNT){
                recommendVideoAdapter.updateList(newDatas, true);
                fromIndex += newDatas.size();
                toIndex += newDatas.size();
            }
            else {
                recommendVideoAdapter.updateList(newDatas, false);
            }
